library(glmnet)
FAME <- function(sample_size, x, y, subdata_num, dim, random_seed, index_beta){
  ...
  # t-value statistic using cpt to multiple changepoint detection
  t_value = c()
  for(i in c(1 : (dim(x.train)[2]-1))){
    model = lm(y.train ~ x.train[,i])
    model_summary = summary(model)
    model_coefficient = model_summary$coefficients
    if (dim(model_coefficient)[1] != 1){
      t_value_x = model_coefficient[2, 3]
    } else {
      t_value_x = 0}
    t_value <- c(t_value, abs(t_value_x))
  }
  t_value.order <- order(t_value, decreasing = TRUE)
  
  for (i in 1:dim){
    if (i == 1){
      x.temp <- x.train[ , c(t_value.order[i], de)]                
    } else {
      x.temp <- x.train[-x.index, c(t_value.order[i], de)]         
    }
  ...
  z.fame <- (x.train[x.index, t_value.order[1:dim]])      
  ...
  predict.fame <- predict(lasso.fame, s = lambda.fame, newx = x.test[,t_value.order[1:dim]])   
  t2 <- Sys.time()
  
  # measurement
  mse.fame <- mean((y.test - predict.fame)^2)
  choose_beta <- t_value.order[which(lasso.fame$beta != 0)]
