IBOSS <- function(sample_size, x, y, subdata_num, dim, random_seed, index_beta, tr){
  ## Given original dataset, return MSE of linear regression on subdata selected by IBOSS method
  ...
  t1 <- Sys.time()
  lm.iboss <- lm(y.iboss ~ z.iboss)  
  z.iboss <- x.test
  predict.iboss <- predict(lm.iboss, as.data.frame(z.iboss))
  t2 <- Sys.time()
  
  # measurement
  mse.iboss <- mean((y.test - predict.iboss)^2)  
  choose_beta <- which(lm.iboss$coefficients != 0)
  tp <- sum(choose_beta %in% index_beta); fp <- length(which(lm.iboss$coefficients != 0))- tp
  fn <- tr - tp; tn <- p - tr - fp
  tp.iboss <- tp/(tp + fn)
  ...
  output_list <- list("mse" = mse.iboss, "lasso_model" = lm.iboss, "train_index" = train_ind,
